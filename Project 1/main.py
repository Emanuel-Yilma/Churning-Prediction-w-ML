...
def make_predictions(input_df, input_dict):
    ...
    probabilities = {
        'XCBoost': xgboost_model.predict_proba(input_df)[0][1],
    ...
    with col1:
        fig = create_gauge_chart(avg_probability)  # Call the function directly
        st.plotly_chart(fig, use_container_width=True)
        st.write(
            f"The customer has a {avg_probability: .2%} probability of churning."
        )
    with col2:
        fig_probs = create_model_probability_chart(
            probabilities)  # Call the function directly
...
- Your explanation should be based on the customer's information, the summary statistics of churned and non-churned customers, and the top 10 most important features for predicting churn.
...
def generate_email(probability, input_dict, explanation, surname):
  ...
  You noticed a customer name {surname} has a {round(probability * 100, 1)}% probability of churning, based on the information provided below.
  ...
  Make sure to list out a set of incentives to stay based on their informaiton in bullet point format. Don't ever mention the probability of churning or the machine learning model to the customer. [Your Name] is Emanuel Yilma
